...
const Chatbot = () => {
  ...
  const addRichMessage = (htmlContent: string) => {
    setMessages(prev => [...prev, { type: 'bot', content: htmlContent, isRich: true }]);
  };
    ...
    const eventHTML = `
      <div>
        ...
        <div class="mt-2">
          ...
          ${event.url ? `<p><strong>URL:</strong> <a href="${event.url}" target="_blank" class="text-blue-500 underline">${event.url}</a></p>` : ''}
        ...
        <div class="mt-2">
          <h5 class="text-md font-medium">Event Image</h5>
          <div class="flex gap-2 flex-wrap">
            <div>
              <img src="${event.imageUrl}" alt="${event.title}" class="w-24 h-24 object-cover cursor-pointer" 
                   onclick="window.dispatchEvent(new CustomEvent('openLightbox', { detail: '${event.imageUrl}' }))">
            </div>
          </div>
          ...
          <button class="mt-2 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600" 
                  onclick="window.dispatchEvent(new CustomEvent('navigateToPayment', { detail: '${event._id}' }))">
            Buy
          </button>
        </div>
      </div>
    `;
    addRichMessage(eventHTML);
  };
  ...
  useEffect(() => {
    ...
    const handleLightbox = (e: CustomEvent) => {
      ...
      lightbox.innerHTML = `
        ...
        <img src="${e.detail}" alt="Enlarged event photo" class="max-w-[90%] max-h-[90%]">
      `;
      document.body.appendChild(lightbox);
      lightbox.addEventListener('click', (ev) => ev.target === lightbox && lightbox.remove());
    };

    const handleNavigateToPayment = (e: CustomEvent) => {
      const eventId = e.detail;
      router.push(`/payment/${eventId}`);
    };

    window.addEventListener('sendMessage', handleCustomSend as EventListener);
    window.addEventListener('openLightbox', handleLightbox as EventListener);
    window.addEventListener('navigateToPayment', handleNavigateToPayment as EventListener);

    return () => {
      window.removeEventListener('sendMessage', handleCustomSend as EventListener);
      window.removeEventListener('openLightbox', handleLightbox as EventListener);
      window.removeEventListener('navigateToPayment', handleNavigateToPayment as EventListener);
    };
  }, [router]);
              ...
              <div key={index} className="mb-2">
                <div 
                  className={`p-2 rounded-lg max-w-[80%] ${
                    msg.type === 'user' ? 'bg-blue-500 text-white ml-auto' : 'bg-gray-100 text-black'
                  }`}
                  dangerouslySetInnerHTML={{ __html: msg.isRich ? msg.content : msg.content }}
                />
              </div>
            ))}
            {isGeminiLoading && (
